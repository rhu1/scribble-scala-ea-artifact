...
trait ActorA extends Actor {
    ...
    def registerA[D <: Session.Data](port: Int, apHost: String, apPort: Int, d: D, f: (D, A1) => Done.type): Unit = {
        ...
        enqueueRegisterForPeers(apHost, apPort, "Hello", "A", port, d, f, g, Set("B"))
    }
}
    ...
    def suspend[D <: Session.Data](d: D, f: (D, A2) => Done.type): Done.type = {
        ...
        val g = (op: String, pay: String) => {
            ...
            if (op == "L2") {
                ...
                succ = Some(s)
                val split = pay.split("::::")
                L2A(sid, role, actor.deserializeInt(split(0)), actor.deserializeString(split(1)), actor.deserializeBoolean(split(2)), s)
            } else {
...
case class L2A(sid: Session.Sid, role: Session.Role, x1: Int, x2: String, x3: Boolean, s: EndA) extends A2