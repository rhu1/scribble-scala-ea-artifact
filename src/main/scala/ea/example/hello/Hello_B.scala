...
trait ActorB extends Actor {
    ...
    def registerB[D <: Session.Data](port: Int, apHost: String, apPort: Int, d: D, f: (D, B1Suspend) => Done.type): Unit = {
        ...
        enqueueRegisterForPeers(apHost, apPort, "Hello", "B", port, d, f, g, Set("A"))
    }
}
...
case class B2(sid: Session.Sid, role: Session.Role, actor: Actor) extends Session.OState[Actor] {
    ...
    @throws[IOException]
    def sendL2(x1: Int, x2: String, x3: Boolean): EndB = {
        checkNotUsed()
        val pay = actor.serializeInt(x1) + "::::" + actor.serializeString(x2) + "::::" + actor.serializeBoolean(x3)
