...
trait ActorPinger extends Actor {
    ...
    def registerPinger[D <: Session.Data](port: Int, apHost: String, apPort: Int, d: D, f: (D, Pinger1Suspend) => Done.type): Unit = {
        ...
        enqueueRegisterForPeers(apHost, apPort, "PingPong1", "Pinger", port, d, f, g, Set("C", "Ponger", "PongReceiver"))
    }
}
...
case class PingCPinger(sid: Session.Sid, role: Session.Role, s: Pinger2) extends Pinger3

case class StopPinger(sid: Session.Sid, role: Session.Role, s: Pinger4) extends Pinger3

case class Pinger4(sid: Session.Sid, role: Session.Role, actor: Actor) extends Session.OState[Actor] {

    @throws[IOException]
    def sendStop(): Pinger5 = {
        checkNotUsed()
        val pay = ""
        actor.sendMessage(sid, "Pinger", "Ponger", "Stop", pay)
        Pinger5(sid, "Pinger", actor)
    }
}
