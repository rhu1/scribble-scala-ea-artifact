...
trait ActorPonger extends Actor {
    ...
    def registerPonger[D <: Session.Data](port: Int, apHost: String, apPort: Int, d: D, f: (D, Ponger1Suspend) => Done.type): Unit = {
        ...
        enqueueRegisterForPeers(apHost, apPort, "PingPong1", "Ponger", port, d, f, g, Set("C", "Pinger", "PongReceiver"))
    }
}
...
case class PingPonger(sid: Session.Sid, role: Session.Role, s: Ponger2) extends Ponger1
...
case class Ponger2(sid: Session.Sid, role: Session.Role, actor: Actor) extends Session.OState[Actor] {
    ...
    def sendPong(): Ponger3Suspend = {
        checkNotUsed()
        val pay = ""
        actor.sendMessage(sid, "Ponger", "PongReceiver", "Pong", pay)
...
case class PingPonger3(sid: Session.Sid, role: Session.Role, s: Ponger2) extends Ponger3
