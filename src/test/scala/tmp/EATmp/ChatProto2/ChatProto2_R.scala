...
trait ActorR extends Actor {
	...
	def registerR[D <: Session.Data](port: Int, apHost: String, apPort: Int, d: D, f: (D, R1Suspend) => Done.type): Unit = {
		val g = (sid: Session.Sid) => R1Suspend(sid, "R", this)
		enqueueRegisterForPeers(apHost, apPort, "ChatProto2", "R", port, d, f, g, Set("C"))
	}
}

case class R1Suspend(sid: Session.Sid, role: Session.Role, actor: Actor) extends Session.SuspendState[Actor] {
	...
	def suspend[D <: Session.Data](d: D, f: (D, R1) => Done.type): Done.type = {
		...
		val g = (op: String, pay: String) => {
			...
			if (op == "OutgoingChatMessage") {
				val s = R1Suspend(sid, role, actor)
				succ = Some(s)
				val split = pay.split("::::")
				OutgoingChatMessageR(sid, role, actor.deserializeString(split(0)), s)
			} else 	if (op == "LeaveRoom") {
				val s = EndR(sid, role, actor)
				succ = Some(s)
				val split = pay.split("::::")
				LeaveRoomR(sid, role, actor.deserializeString(split(0)), s)
			} else {
...
case class OutgoingChatMessageR(sid: Session.Sid, role: Session.Role, x1: String, s: R1Suspend) extends R1

case class LeaveRoomR(sid: Session.Sid, role: Session.Role, x1: String, s: EndR) extends R1
