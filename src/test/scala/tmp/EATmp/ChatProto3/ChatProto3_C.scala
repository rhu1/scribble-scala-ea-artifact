	...
	def suspend[D <: Session.Data](d: D, f: (D, C1) => Done.type): Done.type = {
		...
		val g = (op: String, pay: String) => {
			...
			if (op == "IncomingChatMessage") {
				val s = C1Suspend(sid, role, actor)
				succ = Some(s)
				val split = pay.split("::::")
				IncomingChatMessageC(sid, role, actor.deserializeString(split(0)), s)
			} else 	if (op == "Bye") {
				val s = EndC(sid, role, actor)
				succ = Some(s)
				val split = pay.split("::::")
				ByeC(sid, role, actor.deserializeString(split(0)), s)
			} else {
...
case class IncomingChatMessageC(sid: Session.Sid, role: Session.Role, x1: String, s: C1Suspend) extends C1

case class ByeC(sid: Session.Sid, role: Session.Role, x1: String, s: EndC) extends C1
